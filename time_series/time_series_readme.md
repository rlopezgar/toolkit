...
* Retail: Sales might peak during holiday seasons and drop afterward.
* Agriculture: Crop yields vary with growing seasons.
* Tourism: Certain destinations may see more visitors during specific seasons.
  
Understanding seasonality helps businesses and analysts make informed decisions, plan for peak times, and manage inventory or resources effectively.
  
A seasonal time series is time series which exhibits predictable and recurring patterns or fluctuations that follow a specific season. These patterns repeat at regular intervals, often corresponding to  months, or quarters or  semesters within a year.

We'll speak generally of a seasonal period that includes $E$ contiguous observations within each cycle where:
* $E = 12$ for monthly seasonality. 
* $E = 6$ for biannual seasonality.
...
Let \{Z_t\} be a time series with seasonal period $E$. The seasonal difference operator $\nabla_{E}^{D}$ $(k\in\mathbb{N})$ is such that: 
$$\nabla_{E}^{k}Z_t = (1-B^E)^kZ_t \\
                    = \sum_{j = 0}^k\frac{k!}{j!(k-j)!}(-1)^{j}Z_{t-jE}$$
Must note that a seasonal difference operator of order $k$ $\nabla_{E}^D $automatically errases $E\times D$ observations of the time series. 
...
**Seasonal Backshift Polynomial**  
Let \{Z_t\} be a time serie with seasonal period $E$, $g_0 = -1, g_1, g_2, ..., g_k \in\mathbb{R}$. A seasonal backshift polynomial  $G(B^E)$  of order $k$ for $k \in \mathbb{N}\cup\{0\}$ is as follows:
$$G(B^E) = 1 - g_1B^E-g_2B^{2E}-...-g_kB^{kE}\\
         = 1-\sum_{j=1}^kg_jB^{jE}$$
...
Just and differnciated ARIMA in order for the time series to be stationary.
...
Note that the model has to be estimated in order to ensure its stationarity and invertibility. 
...
$$\Phi(B^E)\nabla_{E}^{D}(Z_t) = \Theta(B^E)a_t$$
Where:
* $\Phi(B^E) = 1-\Phi_1B^E - \Phi_2B^{2E} - ...- \Phi_PB^{PE}$ is a seasonal backshift polynomial of order $Q$ that represents the autoregressive part of the seasonal model. Thus, the current value of a time series is regressed on its own $Q$ previous seasonal values. This is, $Z_{t-E}, Z_{t-2E}, ..., Z_{t-QE}$.
* $\Theta(B^E) = 1-\Theta_1B^E - \Theta_2B^{2E} - ...- \Theta_PB^{PE}$ is a seasonal backshift polynomial of order $P$ that represents the moving averages part of the seasonal model. Thus, the current value of a time series can be expressed as a linear combination of $Q$ past seaosnal error terms. This is, the terms $a_{t-E}, a_{t-2E}, ..., a_{t-PE}$.
* \nabla_{E}^{D} is an seaosnal difference operator of order $D$.
* $a_t$ is a white noise porcess.

**$SARIMA(p,d,q)(P,D,Q)_E$**   
**Multiplicative Seasonal Autoregressive Integrated Moving Averages Model**  
Let \{Z_t\} be a time series with seasonal period $E$. An purely seasonal autorregresive integrated moving averages model is expressed as follows:
$$\phi(B)\Phi(B^{E})\nabla^d\nabla^D_E(Z_t) = \theta(B)\Theta(B^{E})a_t$$
Where:
...
The greatness of the multiplicative seasonal autorregresive integrated moving averages model is that, for example,  for a time series with monthly observations, we'll be able to catch the following:
* Relations between contiguous months in the same year.
* Relations between same months in different years.  
